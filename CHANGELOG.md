...
- **Гео-посадочные ЮЗАО и ЗАО** (ЧТЗ_SEO_Рост_позиций_Вебмастер_v2): 24 района + 2 хаба
  (`evakuator-yuzao-moskvy`, `evakuator-zao-moskvy`) — гео-модуль вырос до 102 страниц
  (94 локации + 8 хабов, 109 посадочных суммарно). Меты содержат обе формы запросов:
  разговорные («эвакуатор ясенево») в title/H1, литературные («эвакуатор в Ясенево»)
  в description/FAQ; sitemap — приоритеты 0.7/0.8. Список URL для переобхода — SEO_SETUP.md.

### Изменено
- **Лимит 15 тонн** (ЭПИК-2): карточка «Эвакуация спецтехники» и FAQ честно указывают
  «до 15 тонн»; упоминания грузовиков/автобусов убраны (faq.ts, services.ts,
  moscow-yuvao.ts — «грузовые развязки» → «транспортные»). Автотест-«сторож»
  (forbidden-словарь) не даст вернуться грузовой тематике.

### Исправлено
- E2E (существующие, не проходили после 98cb464): крошки Горьковки ищут актуальный
  текст хаба «востоке Подмосковья»; локаторы блоков «Районы …» — по началу заголовка
  (конфликт с FAQ-вопросом хабов).

## [0.12.1] — 2026-09-06

### Добавлено
- **Верификация Google Search Console** (1b9c453): служебная страница `google0f696e1033e85b54.html` + автотест.
